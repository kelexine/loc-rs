...
impl Extractor for NimExtractor {
    fn extract(&self, content: &str) -> Vec<FunctionInfo> {
        super::with_parsed_tree(tree_sitter_nim::language(), content, |tree| {
...
fn traverse(node: Node, content: &str, lines: &[&str], functions: &mut Vec<FunctionInfo>) {
    ...
    if matches!(
        ...
        for child in node.children(&mut cursor) {
            ...
            } else if child.kind() == "object_declaration"
                || child.kind() == "ref_object_declaration"
            {
                is_class = true;
            }
        }
...
fn parse_function(
    ...
    Some(FunctionInfo {
        ...
        is_class: false,
        docstring: None,
        decorators: if is_exported { vec!["public(*)".into()] } else { vec![] },
        complexity,
    })
}
